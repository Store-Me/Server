...
import com.example.storeme.global.common.annotation.CustomStoreNumber;
...
public class CustomStoreNumberValidator implements ConstraintValidator<CustomStoreNumber, JsonNullable<String>> {
    // 지역번호를 정의
    ...
    @Override
    public boolean isValid(JsonNullable<String> input, ConstraintValidatorContext context) {
        if (input == null || input.get() == null)
            return true;

        // 안심번호 검증
        if (Pattern.matches(SAFE_NUMBER_PATTERN, input.get())) {
            return true;
        }

        // 휴대전화번호 검증
        else if (Pattern.matches(MOBILE_PHONE_PATTERN, input.get())) {
            return true;
        }

        // 인터넷전화번호 검증
        else if (Pattern.matches(INTERNET_PHONE_PATTERN, input.get())) {
            return true;
        }
        ...
        // 유선전화번호 검증
            ...
            for (AreaCode areaCode : AreaCode.values()) {
                if (Pattern.matches("^" + areaCode.getCode() + LANDLINE_PHONE_PATTERN, input.get())) {
                    return true;
                }
            }
        }
